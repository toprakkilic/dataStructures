public class Graph<T extends Comparable<T>> {
    ...
    public int inDegree (T VertexID) {
        ...
        public void DFS(T startingVertexId) throws Exception {
            Vertex<T> startVertex1 = findVertex(startingVertexId);
            LinkedList<Vertex<T>> visited = new LinkedList<>();
            GenericStack<Vertex> stk = new GenericStack<>(Vertex.class, vertexCount());
            stk.push(startVertex1);

            while (!stk.isEmpty()) {
                Vertex<T> current = stk.pop();
        ...
        Vertex<T> startVertex = findVertex(startingVertexId);
        LinkedList<Vertex<T>> visited = new LinkedList<>();
        CircularQueue<Vertex> queue = new CircularQueue<>(Vertex.class, vertexCount()+1);
